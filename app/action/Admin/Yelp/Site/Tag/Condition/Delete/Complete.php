<?php
/**
 *  Admin/Customer/Site/Tag/Condition/Delete/Complete.php
 *
 *  @author    Alev Co., Ltd.
 *  @package   Lpo
...
/**
 *  admin_customer_site_tag_condition_delete_completeフォームの実装
 *
 *  @author    Alev Co., Ltd.
 *  @access    public
 *  @package   Lpo
 */
class Lpo_Form_AdminCustomerSiteTagConditionDeleteComplete extends Lpo_ActionForm
{
...
/**
 *  admin_customer_site_tag_condition_delete_completeアクションの実装
 *
 *  @author     Alev Co., Ltd.
 *  @access     public
 *  @package    Lpo
 */
class Lpo_Action_AdminCustomerSiteTagConditionDeleteComplete extends Lpo_ActionClass
{
    /**
     *  @access private
     *  @var    array  セッション格納領域
    ...
    /**
     *  admin_customer_site_tag_condition_delete_completeアクションの前処理
     *
     *  @access    public
    ...
    function perform()
    {
        try {
            ...
            $where = array('id' => $this->af->get('id'));
            if ($conditionManager->delete($where) === -1) {
                throw new Exception('データベースの削除に失敗しました。');
            }
            $search = array('target_id' => $this->session->get('admin.target.id'));
            if (!$conditionManager->resetNo($search)) {
                throw new Exception('データベースの削除に失敗しました。');
            }
            DB::commit();
        } catch (Exception $e) {
